...
var input = {
  liked: function(userId, itemId, omitUpdate, callback){
    input.unDisliked(userId, itemId, true, function(){
  ...
  disliked: function(userId, itemId, omitUpdate, callback){
    input.unLiked(userId, itemId, true, function(){
  ...
  clearUser: function(userId, omitUpdate, callback){
    ...
    client.del(similaritySet, function() { });
    client.del(recommendedSet, function() { });
        ...
        function(err){
          client.smembers(userDislikedSet, function(err, userDislikes) {
            async.each(userDislikes,
              ...
              function(err){
                client.del(userLikedSet, function() { });
                client.del(userDislikedSet, function() { });
                if (!omitUpdate) {
                  algo.updateUser(userId, function(){
                    callback();
                  });
  ...
  clearItem: function(itemId, omitUpdate, callback){
    ...
    var recommendationsKeys = [config.className, '*', '*', 'recommendedSet'].join(':');
    client.keys(recommendationsKeys, function(err, recommendations) {
      async.each(recommendations,
        function(recommendation, cb){
          client.zrem(recommendation, String(itemId), function(err) {
            cb(); 
          });
      });
    });
