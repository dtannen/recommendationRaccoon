...
var input = {
  ...
  clearUser: function(userId, update, callback){
    ...
    client.smemebers(userLikedSet, function(err, userLikes) {
      async.each(userLikes,
        function(itemId, cb){
          unLiked(userId, itemId, false, function() {
            if (update) {
              algo.updateItem(itemId, function(){
                cb();
              });
        ...
        function(err){
          client.smemebers(userDislikedSet, function(err, userDislikes) {
            async.each(userDislikes,
              function(itemId, cb){
                unDisliked(userId, itemId, false, function() {
                  if (update) {
                    algo.updateItem(itemId, function(){
                      cb();
                    });
  ...
  clearItem: function(itemId, update, callback){
    ...
    client.smemebers(itemLikedSet, function(err, itemLikes) {
      async.each(itemLikes,
        ...
        function(err){
          client.smemebers(itemDislikedSet, function(err, itemDislikes) {
